    ...
    async fn run(&self, command: LifecycleCommand) -> Result<LifecycleOutput> {
        ...
        match command {
            ...
            LifecycleCommand::Stop => {
                ...
                {
                    let settings = DaemonSettings::load_for_stop(&selected.settings_file).await;
                    backend::pid_update_loop_backend(selected.backend_paths(&settings))
                        .stop_with_grace(settings.shutdown_grace_seconds)
                        .await?;
                    match tokio::fs::remove_file(selected.manual_update_socket_path()).await {
                        Ok(()) => {}
