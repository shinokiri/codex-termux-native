...
def prepare(args):
    ...
    for name, (remote, revision) in ANDROID_DEPS.items():
        if revision not in deps:
            raise RuntimeError(f"Android dependency pin absent from upstream DEPS: {name}")
...
def build_v8(args):
    ...
    env.update(
        {
            ...
            "CODEX_V8_CLANG_RESOURCE_DIR": output(["clang-21", "-print-resource-dir"]),
    ...
    run(
        [
            "cargo", f"+{RUST}", "build", "--locked", "--release", "--target", TARGET,
    ...
    if not (gn / "obj/librusty_v8.a").is_file() or not (gn / "src_binding.rs").is_file():
    ...
    env.update(
        {
            ...
            "CARGO_TARGET_AARCH64_LINUX_ANDROID_RUSTFLAGS":
                "-C link-arg=-Wl,-rpath,$ORIGIN/../lib -C link-arg=-Wl,-z,max-page-size=16384",
        }
    )
    run(
        [
            "cargo", f"+{RUST}", "build", "--locked", "--release", "--target", TARGET,
            "-p", "codex-cli", "-p", "codex-code-mode-host", "-p", "codex-http-client",
            "--bins", "--example", "termux_probe",
        ],
        cwd=ROOT / "codex-rs",
        env=env,
    )
...
def package(args):
    ...
    for name in ("codex", "codex-code-mode-host", "examples/termux_probe"):
        binary = release / name
        description = output([readelf, "--file-header", "--program-headers", "--dynamic", binary])
        ...
        supported = {"libc.so", "libm.so", "libdl.so", "liblog.so", "libandroid.so", "libz.so", "libc++_shared.so"}
    ...
    provenance = {
        ...
        "target": TARGET, "minimum_android_api": int(API), "ndk": NDK,
        "rustc": output(["rustc", f"+{RUST}", "--version"]),
        "v8_version": V8_VERSION, "v8_commit": V8_COMMIT,
        "v8_submodules": output(["git", "submodule", "status", "--recursive"], cwd=args.source),
        "android_dependencies": ANDROID_DEPS,
        "v8_gn_args": (gn / "args.gn").read_text(),
        "v8_binding_patch_sha256": digest(ROOT / "patches/termux-rusty-v8-bindgen.patch"),
...
def main():
    ...
    {"prepare": prepare, "v8": build_v8, "codex": build_codex, "package": package}[args.stage](args)
